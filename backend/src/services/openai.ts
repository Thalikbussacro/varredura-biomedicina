...
export class OpenAIService {
  ...
  async generateEmail(params: EmailGenerationParams): Promise<GeneratedEmailContent> {
    const prompt = `
      ...
      - Mencionar interesse específico na área de atuação do estabelecimento (${this.getCategoryDescription(params.category)}), ajustando o nome para não parecer automático
      - Destacar disponibilidade para estágio não remunerado
      - Informar período de estágio: início no segundo semestre 2026, 20 horas semanais, com possibilidade de extensão
